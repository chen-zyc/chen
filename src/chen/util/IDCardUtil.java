...
/**
 * from http://www.oschina.net/code/snippet_1859292_39120
...
public class IDCardUtil {
	...
	public static boolean isIDCard(String certNo) {
		if (certNo == null || (certNo.length() != 15 && certNo.length() != 18)) return false;
		final char[] cs = certNo.toUpperCase().toCharArray();
		// 校验每一位是否是数字或X，并计算权重
		int power = 0;
		for (int i = 0; i < cs.length; i++) {
			if (i == cs.length - 1 && cs[i] == 'X') break;// 最后一位可以 是X或x
		...
		if (iyear < 1900 || iyear > Calendar.getInstance().get(Calendar.YEAR)) return false;// 1900年的PASS，超过今年的PASS
		...
		// 校验"校验码"
		if (certNo.length() == 15) return true;
